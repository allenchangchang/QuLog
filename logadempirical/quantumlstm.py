...
from logadempirical.logdeep.models.lstm import deeplog, quantumlstm
...
def run_qlstm(options):
    if not os.path.exists(options["vocab_path"]):
        ...
        vocab = Vocab(logs, os.path.join(options['data_dir'], "new_embeddings.json"), "quantumlstm")

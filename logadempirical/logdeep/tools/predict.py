...
from logadempirical.logdeep.models.lstm import deeplog, loganomaly, robustlog, quantumlstm, qloganomaly, qrobustlog
...
class Predicter():
    def __init__(self, options):
        ...
        self.n_qubits = options["n_qubits"]
        self.n_qlayers = options["n_qlayers"]
        self.n_vrotations = options["n_vrotations"]
        self.batch_first = options["batch_first"]
        self.return_sequences = options["return_sequences"]
        self.return_state = options["return_state"]
    ...
    def semi_supervised_helper(self, model, logs, vocab, data_type, scale=None, min_len=0):
        ...
        seqs, labels = sliding_window(sess_events, 
                                      vocab=vocab, 
    ...
    def predict_semi_supervised(self):
        ...
        if self.model_name == "deeplog":
            ...
            if self.model_name == "qlstm":
                lstm_model = quantumlstm
            elif self.model_name == "qloganomaly":
                lstm_model = qloganomaly
            model_init = lstm_model(input_size=self.input_size,
                                    hidden_size=self.hidden_size,
                                    num_layers=self.num_layers,
        ...
        test_normal, test_abnormal = generate(self.output_dir, 'test.pkl', False)
    ...
    def predict_supervised2(self):
        ...
        if self.model_name == "cnn":
        ...
        elif self.model_name == "logrobust":
            lstm_model = robustlog
            model_init = lstm_model(input_size=self.input_size,
                                    hidden_size=self.hidden_size,
                                    num_layers=self.num_layers,
        ...
        elif self.model_name == "qlogrobust":
            lstm_model = qrobustlog
            model_init = lstm_model(input_size=self.input_size,
                            hidden_size=self.hidden_size,
                            n_qubits=self.n_qubits,
                            n_qlayers=self.n_qlayers,
            ...
            raise ValueError(f"Predict.py model init has no {self.model_name}")
        
        model_pth = torch.load(self.model_path)
        # dict_keys(['epoch', 'state_dict', 'best_loss', 'log', 'best_score'])
        # print((model_pth['state_dict']))
        # strict=false 过滤掉不匹配的权重
        pretrained_dict = model_pth['state_dict']
        model_dict = model.state_dict()
        # 过滤掉形状不匹配的参数
        filtered_pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and v.shape == model_dict[k].shape}
        # 更新当前模型的 state_dict

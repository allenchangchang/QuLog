...
#             f_t = torch.sigmoid(self.clayer_out(self.VQC['forget'](y_t,self.dev)))  # forget block
#             i_t = torch.sigmoid(self.clayer_out(self.VQC['input'](y_t,self.dev)))  # input block
#             g_t = torch.tanh(self.clayer_out(self.VQC['update'](y_t,self.dev)))  # update block
#             o_t = torch.sigmoid(self.clayer_out(self.VQC['output'](y_t,self.dev))) # output block
...
class QLSTM(nn.Module):
    def __init__(self, 
        ...
        # self.dev_forget = qml.device(self.backend, wires=self.wires_forget, gpu=True)
        # self.dev_input = qml.device(self.backend, wires=self.wires_input, gpu=True)
        # self.dev_update = qml.device(self.backend, wires=self.wires_update, gpu=True)
        # self.dev_output = qml.device(self.backend, wires=self.wires_output, gpu=True)

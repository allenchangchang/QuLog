...
def process_dataset(data_dir, output_dir, log_file, dataset_name, window_type, window_size, step_size, train_size,
    ...
    if window_type == "sliding":
        ...
        elif 'spirit' in dataset_name:
            df['datetime'] = pd.to_datetime(df["Date"] + " " + df['Time'], format='%Y.%m.%d %H:%M:%S')
        elif 'tbird' in dataset_name:
            df['datetime'] = pd.to_datetime(df["Date"] + " " + df['Time'], format='%Y.%m.%d %H:%M:%S')

        elif 'hdfs' in dataset_name:
            df['Date'] = df['Date'].astype(str)
            df['Time'] = df['Time'].astype(str)
            df['datetime'] = pd.to_datetime(df.apply(lambda row: row['Date'][:2] + '-' + row['Date'][2:4] + '-' + row['Date'][4:] + ' ' + row['Time'][:2] + ':' + row['Time'][2:4] + ':' + row['Time'][4:], axis=1), format='%Y-%m-%d %H:%M:%S')
        else:
            df['datetime'] = pd.to_datetime(df["Date"] + " " + df['Time'], format='%Y-%m-%d %H:%M:%S')
